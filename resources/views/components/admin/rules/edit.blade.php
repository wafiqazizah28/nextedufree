...
<div class="w-full self-center px-4">
  <div class="self-center">
          ...
          <thead class="text-slate-700">
            <tr>
              ...
              <th class="border bg-slate-50 px-6 py-3 font-bold uppercase">
                <span class="font-semibold text-secondary">{{ $jurusanDetail['jurusan_code'] }}</span> -
                {{ $jurusanDetail['name'] }}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr class="px-6 py-3 text-center">
                ...
                <td class="border px-6 py-2 text-justify"><span
                    class="font-semibold text-secondary">{{ $pertanyaanList[$i]['symptoms_code'] }}</span><br>{{ $pertanyaanList[$i]['pertanyaan'] }}
                </td>
                <td class="border px-6 py-2">
                  <select name="options" id="options-{{ $i }}" class="w-full rounded-md"
                    onchange="updateRuleListener(this, {{ $jurusanDetail['id'] }}, {{ $pertanyaanList[$i]['id'] }})">
                    @if ($jurusanDetail['rules'][$i] == 0)
    ...
    <script>
      ...
      const updateRuleListener = (e, jurusanId, pertanyaanId) => {
        const value = e.value;

        for (let i = 0; i < rules.length; i++) {
          if (rules[i].jurusanId === jurusanId && rules[i].pertanyaanId === pertanyaanId) {
