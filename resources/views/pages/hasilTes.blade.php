...
@section('content')
<section class="pt-14 pb-12 lg:pt-24 bg-gray-50 min-h-screen">
    <div class="container mx-auto max-w-4xl px-4">

        {{-- Card Hasil Tes --}}
        <div class="bg-white shadow-md rounded-xl p-6 flex flex-col md:flex-row items-center gap-6">
            ...
            <div class="flex-1 text-center md:text-left">
                <h2 class="text-xl font-bold text-purple-700 bg-purple-100 px-4 py-2 rounded-md inline-block">
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    <a href="{{ route('hasil-tes.download', $hasilTes->id) }}" class="px-4 py-2 bg-green-600 text-white rounded-md flex items-center gap-2 hover:bg-green-700 transition-colors text-sm font-medium">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        Unduh PDF
                    </a>
                    <button type="button" onclick="openShareModal()" class="px-4 py-2 bg-green-600 text-white rounded-md flex items-center gap-2 hover:bg-green-700 transition-colors text-sm font-medium">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div id="shareModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
            <div class="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 shadow-2xl">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onclick="closeShareModal()" class="text-gray-500 hover:text-gray-700 focus:outline-none">
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    <!-- Image Preview -->
                    <div class="w-full md:w-1/2 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center p-2 border">
                        <img id="shareImagePreview" src="/images/loading.gif" alt="Preview hasil tes" class="max-w-full h-auto">
                    </div>
                    
                    <!-- Share Options -->
                    <div class="w-full md:w-1/2">
                        <p class="mb-4 text-gray-600">Bagikan hasil tes jurusan {{ $hasilTes->hasil ?? '' }} ke platform sosial media:</p>
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-2 text-sm text-gray-500 text-center">Simpan gambar dan unggah ke Instagram Stories atau Post</p>
                        </div>
                        
                        <div class="pt-3 border-t">
                            <div class="relative mb-3">
                                <input type="text" id="shareLink" value="{{ route('hasil-tes.show', $hasilTes->id) }}" 
                                       class="w-full p-2 pr-20 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm" readonly>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @php
                // Ambil semua jurusan jika $jurusanList tidak tersedia
                if(!isset($jurusanList)) {
                    $jurusanList = \App\Models\Jurusan::all();
                }
                
                // Cari jurusan berdasarkan hasil tes
                $jurusanTerkait = null;
                ...
                // Filter saran pekerjaan berdasarkan jurusan
                $saranPekerjaanList = [];
                if($jurusanTerkait) {
            ...
            @if(count($saranPekerjaanList) > 0)
                @foreach($saranPekerjaanList as $saranPekerjaan)
                    <div class="bg-white shadow rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="h-48 overflow-hidden">
                            @if($saranPekerjaan->gambar)
                                <img src="{{ asset('storage/' . $saranPekerjaan->gambar) }}" 
                                     alt="{{ $saranPekerjaan->saran_pekerjaan }}"
                        ...
                        <div class="p-4">
                            <h4 class="text-lg font-semibold text-purple-700">{{ $saranPekerjaan->saran_pekerjaan }}</h4>
        ...
        {{-- Rekomendasi Sekolah --}}
        <h3 class="mt-16 text-lg font-semibold text-gray-700 text-center">Rekomendasi Sekolah untuk jurusan ini:</h3>
        ...
        <div class="mt-6 bg-white shadow-md rounded-lg overflow-hidden">
            @php
                ...
                $sekolahList = [];
                if($jurusanTerkait) {
                    $sekolahList = \App\Models\Sekolah::where('jurusan_id', $jurusanTerkait->id)->orderBy('nama', 'asc')->get();
                }
            @endphp
                        ...
                        <li class="p-4 hover:bg-gray-50 transition-colors">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-sm text-gray-500">Program Jurusan: {{ $jurusanTerkait->jurusan }}</p>
                                </div>
                            </div>
                        </li>
                    @endforeach
    ...
    {{-- Testimoni Section --}}
<div class="mt-16 bg-white shadow-md rounded-xl p-6">
    <h3 class="text-lg font-semibold text-gray-700 text-center mb-6">Bagikan Pengalaman Anda</h3>
    ...
    <form action="{{ route('testimoni.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea id="testimoni" name="testimoni" rows="4"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200 focus:ring-opacity-50 @error('testimoni') border-red-500 @enderror"
        ...
        <div class="flex justify-end">
            <button type="submit" class="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2">
                Kirim Testimoni
            </button>
        </div>
    </form>
